...
def make_xml(cal_items,  filename, recordername = "Lecture Room 2"):
    ...
    #exclude calendar items that occur before today
        ...
        rec_sched = recorder_sched.format(PRESENTATIONTITLE = html_escape(item.short_name()), PRESENTERS= ''.join(presentersxml), FOLDERNAME= html_escape(item.mediasite_fldr), RECORDERNAME=html_escape(recordername), BEGINDATE=begindate, ENDDATE=enddate, DURATION= (item.get_rec_end() - item.start_date).seconds/60)
