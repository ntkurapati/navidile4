...
def update_settings(reschedule=False):
    ...
    if not os.path.exists(yamlfile):
        import shutil
        shutil.copyfile('navidile_settings_template.yml', yamlfile)
    ...
    if 'notify_updates' not in settings[hostname]:
        settings[hostname]['notify_updates']=False
    if 'log_loc' not in settings[hostname]:
        settings[hostname]['log_loc']=tempdir
    if 'default_dir' not in settings[hostname]:
        settings[hostname]['default_dir']=tempdir
    ...
    if 'zone_stuff_loc' not in settings[hostname]:
        settings[hostname]['zone_stuff_loc']=tempdir
    ...
    if 'email_in_port' not in settings[hostname]:
        settings[hostname]['email_in_port']= 2524
    if 'refresh_rates' not in settings[hostname]:
        settings[hostname]['refresh_rates']={ 'courses' : 900, 'grades': 900, 'recordings': 300, 'webpages': 300, 'course_cals': 300, 'subscriptions' : 30 }
        
    yaml.dump(file(yamlfile))
   
    if reschedule:
        schedule.enter(60, 1, update_settings, ())
...
# set up the logger
def init_logger(loggername, filename =None):
    if not filename:
...
def main(argv):
    ...
    global settings
    update_settings(reschedule=False)
    ...
    for i in range(1,1000):
        ...
        for task in tasks:
            if not task.last_ran or (task.last_ran+datetime.timedelta(seconds=task.run_interval))<datetime.datetime.now() or task.force_run:
            
            #getattr(self, task.name)(task)
            
                if task.name== "update_webpages": update_webpages(task)
                elif task.name== "update_calendars": update_calendars(task)
                elif task.name== "update_subscribers": update_subscribers(task)
                elif task.name== "update_navidile_players": update_navidile_players(task)
                elif task.name== "update_mediasite_sched": update_mediasite_sched(task)
                elif task.name== "update_course_docs": update_course_docs(task)
                elif task.name== "update_course_db": update_course_db(task)
                elif task.name== "update_recordings": update_recordings(task)
                elif task.name== "redundancy_check": redundancy_check(task)
...
def removeNonAscii(line): 
    #import unicodedata
    #return unicodedata.normalize('NFKD', line).encode('ascii','ignore').strip()
...
def updatecalendar(msclass):
    logger.info('Updating  Calendar for {0}:'.format(msclass.cyear));
    
    #retrieve relevant calendar items from database
    calitems = s.query(CalendarItem).filter(CalendarItem.cyear == msclass.cyear).all()
    
    if len(calitems)==0:
        logger.warn('No calendar items!')
        calstring = """
BEGIN:VCALENDAR
VERSION:2.0
...
"""
        ...
        cal.add('version', '2.0')
        cal.add('X-WR-CALNAME', 'PITTMED%s' % msclass.cyear)
        cal.add('X-WR-CALDESC', 'PITTMED%s Navigator Calendar' %msclass.cyear)
        cal.add('X-WR-TIMEZONE', 'America/New_York')
        
        # is there a template already?  If so, just start with that instead...
        if msclass.google_calendar_template:
            try:
                editableversion = msclass.google_calendar_template
                filehandle = urllib.urlopen(editableversion)
                data = filehandle.read();

                cal = icalendar.Calendar.from_ical(data)
                cal.set('prodid', '-//Navigator Calendar//navigator.medschool.pitt.edu//')
                cal.set('version', '2.0')
                cal.set('X-WR-CALNAME', 'PITTMED%s Navigator' %msclass.cyear)
                cal.set('X-WR-CALDESC', 'PITTMED%s Navigator Calendar' %msclass.cyear)
        ...
        calstring = cal.to_ical().replace(';VALUE=DATE', '').replace('-TIME', '')
    
    fname = str(msclass.cyear)+'_navi.ics'
    fileloc=os.path.join(settings[hostname]['cal_location'], fname)
    ...
    f = open(fileloc.replace('.ics', '.txt'), 'wb')
    f.write(calstring)
    f.close()
    logger.info('Done Updating  Calendar for {0}:'.format(msclass.cyear));
...
def redundancy_check(task):
    ...
    for msclass in s.query(MSClass).all():
        ursr = s.query(ScheduledRecording).filter(ScheduledRecording.recorded ==False, \
                                                  ...
                                                  ScheduledRecording.end_date< (datetime.datetime.now()-datetime.timedelta(minutes=60)), \
                                                  ScheduledRecording.end_date> (datetime.datetime.now()-datetime.timedelta(days=3)), \
                                                  ScheduledRecording.cyear ==msclass.cyear,
                                                  ScheduledRecording.notified_unrecorded==False).all()
        if ursr:
            warningtxt = ('Hi, the following lecture(s) did not appear to  record:' ,)                                           
            for u in ursr:
                warningtxt+= (u.l0name,)
                u.notified_unrecorded=True

                s.commit()
            warningtxt +="Please ignore if they weren't supposed to be recorded! Or maybe they went into the wrong course???  Fix in phpmyadmin!",
            warning = NavidileWarning('Missing recording?', '\n'.join(warningtxt), msclass.cyear)
            s.add(warning)
            s.commit()
        ...
        ussr = s.query(ScheduledRecording).filter(ScheduledRecording.scheduled ==False, \
                                                  ScheduledRecording.excluded==False, \
                                                  ScheduledRecording.start_date< (datetime.datetime.now()-datetime.timedelta(days=4)), \
                                                  ScheduledRecording.start_date> datetime.datetime.now(), \
                                                  ScheduledRecording.cyear ==msclass.cyear,
                                                  ScheduledRecording.notified_unscheduled==False).all()
                                                   
        if ussr:
            warningtxt = ('Hi, the following lecture(s) have not been scheduled:' ,)                                           
            for u in ussr:
                warningtxt+= u.l0name,
                u.notified_unscheduled=True
                s.commit()
            warningtxt +="Please ignore if they aren't supposed to be recorded!",
            warning = NavidileWarning('Missing podcast?', '\n'.join(warningtxt),msclass.cyear, tonotify=False)
            s.add(warning)
            s.commit()
        ...
        np = s.query(Recording).filter(Recording.podcast_url =="", \
                                              ...
                                              Recording.cyear ==msclass.cyear, \
                                              Recording.date_added < (datetime.datetime.now() - datetime.timedelta(minutes=7*60)) ) .all()

                                               
        if np:
            warningtxt = ("I couldn't find the podcast for the following lecture(s)" ,)                                           
            for u in np:
                warningtxt+= u.name,
                s.commit()
                u.notified_no_podcast=True
            warningtxt +="Have you checked if the rss feed is set to more than 10 items?  Is the podcast server still running?",
            warning = NavidileWarning('Missing podcast?', '\n'.join(warningtxt),msclass.cyear)
            s.add(warning)
            s.commit()
...
def update_zone_calendar():
    ...
    try:
        ...
        for item in feed["items"]:
            ...
            name = removeNonAscii(item.title)
            idno = item.link[-4:].strip('=')
            description = removeNonAscii(item.description)
            ...
            for tag in tags:
                ...
                if 'Start Time:' in tag.text:
                    ...
                    location = removeNonAscii(tag.text).replace('Location:',"")
                    #print location
    ...
    for year in range(current_year - 1, current_year +2):
        ...
        for ncourse in ncourses:
            ...
            if len(cyears)==1 and ncourse['displayName'] and ncourse['startDate'] and not ncourse['isPlaceholder']:
                ...
                if not course:
                    course = Course(ncourse['displayName'], cyear, course_id=ncourse['moduleID'] , auto_number=False, keep_updated=False)
                if not course.course_id and not course.navigator_url:
                    course.course_id=ncourse['moduleID']
                    course.navigator_url= "http://navigator.medschool.pitt.edu/CourseOverview.aspx?moduleID={0}".format(course.course_id)
                ...
                if len(s.query(Course).filter(Course.cyear==course.cyear, Course.course_id==course.course_id).all())==0:
                    #s.add(course)
                    #s.commit()
                    pass
...
def update_course_docs(task):
    ...
    for course in s.query(Course).filter(Course.navigator_url!=None).all():
        ...
        if not course.course_id==0  and 'viewModule' in course.navigator_url:
            course.navigator_url="http://navigator.medschool.pitt.edu/courseOverview.aspx?moduleID="+str(course.course_id)
...
def update_recordings(task):
    ...
    for course in s.query(Course).filter(Course.mediasite_url!=None).all():
        ...
        count = len(s.query(Recording).filter(Recording.course_name== course.name, Recording.cyear == course.cyear).all())
        print count
        print course.name
        if 'ALL COURSES' not in course.name and (course.keep_updated or count==0 or settings[hostname]['force_update_all']):
...
def update_navidile_players(task):
    ...
    for course in courses:
        count =len(s.query(Recording).filter(Recording.course_uid== course.unique_id, Recording.navidile_url!="").all())
...
def process_request(mailfrom, rcpttos, email_msg):
    ...
    try:
            ...
            if 'forceupdate' in mailto1:
                ...
                msg = MIMEText("okay, I updated the schedules... Go look here: http://students.medschool.pitt.edu/cal -Navidile")
...
def update_navidile_player(course, task):
    ...
    for item in feed["items"]:
        ...
        if  rec:
            ...
            rec.navidile_url = "{0}/{1}/{2}/{3}.html".format(settings[hostname]['host_loc'], course.cyear, course.name,  rec.idno)
...
def make_navidile_player(course, rec, last_rec_url, next_rec_url, task):
    ...
    scripturl = 'http://mediasite.medschool.pitt.edu/som_mediasite/FileServer/Presentation/{0}/manifest.js'.format(rec.idno)    
    refs=[]
    slidebaseurl=''
    ...
    data = f.read()
    f.close()
    data=data.replace('%SLIDEBASEURL%', slidebaseurl).replace('%MP3URL%', rec.podcast_url).replace('%REFS%', repr(refs));
...
def update_subscriber(subscriber, task):
    ...
    if 'r' in subscriber.subscriptions:
        ...
        for course  in s.query(Course).filter(Course.keep_updated==True).all():
            updatedrecs =s.query(Recording).filter(Recording.date_added>subscriber.last_update).filter(Recording.course_name==course.name).filter(Recording.cyear==subscriber.cyear).all()
            if course.keep_updated and len(updatedrecs)>0:
                messagelines=[]                
                construct_vids_message(messagelines, updatedrecs, subscriber)
                send_out_update("\n".join(messagelines), mailfrom,  subscriber, '[Navidile] %s: Recordings Added' %( course.name))
    if 'c' in subscriber.subscriptions:
        mailfrom='alerts%s-c@navidile.mine.nu' % subscriber.cyear
        for course   in s.query(Course).all():

            updateddocs =s.query(Document).filter(Document.date_added>subscriber.last_update).filter(Document.course_name==course.name).filter(Document.cyear==subscriber.cyear).all()
            if course.keep_updated and len(updateddocs)>0:
                messagelines=[]                
                construct_docs_message(messagelines, updateddocs, subscriber)
                send_out_update("\n".join(messagelines), mailfrom,  subscriber, '[Navidile] %s: Documents Added' %( course.name))    
    if 'w' in subscriber.subscriptions:
        mailfrom='alerts%s-w@navidile.mine.nu' % subscriber.cyear
        for warning   in s.query(NavidileWarning).filter(NavidileWarning.cyear==subscriber.cyear, NavidileWarning.date_added > subscriber.last_update).all():
...
def send_out_update(output, mailfrom, subscriber, header):
    email_text= output
    msg = MIMEText(removeNonAscii(email_text))
    msg['Subject'] =  header 
    msg['From'] = mailfrom
    msg['To'] = subscriber.email_addr
    if settings[hostname]['notify_updates']:
        servertools.send_out( 'alerts@students.medschool.pitt.edu', [subscriber.email_addr], msg, settings)
    else:
        logger.info('skip send to %s' % subscriber.email_addr)
...
def check_for_new_recordings(course):
    ...
    if course.rec_count==0:
        newurl = "http://mediasite.medschool.pitt.edu/som_mediasite/Catalog/pages/rss.aspx?catalogId={0}".format(course.mediasite_id)
    ...
    for item in feed["items"]:
        ...
        if not rec:
            rec = Recording(idno1, name =removeNonAscii(item["title"]), mediasite_url=item["link"], course=course ,  pub_date ="")
        rec.rec_date = datetime.datetime.fromtimestamp(time.mktime(item.published_parsed))
        #print item.published_parsed

    
        sr = s.query(ScheduledRecording).filter(ScheduledRecording.l0name == rec.name, ScheduledRecording.course_name==course.name).first()
        if not sr:
            sr = s.query(ScheduledRecording).filter(ScheduledRecording.lname == rec.name, ScheduledRecording.course_name==course.name).first()
        if not sr:
        ...
        if not folder: 
            folder = s.query(Folder).filter(Folder.startDate == rec.rec_date).filter(Folder.course == course.name).filter(Folder.cyear==rec.cyear).first()
        if folder:       
            ...
            if 'virtualHomeFolder' not in  folder['displayName'] :
                ...
                for page in nav4api.folder_pages(course.course_id, folder['folderID'], opener):
                    ...
                    if start_time and end_time:
                        ...
                        ci.presenters = removeNonAscii( re.sub('<[^>]*>', '',  page['source']).replace('and ', '; ').replace('   ', ';').replace('\n', ' '))
                        ...
                        if not exclude and not recording_event.combined_with_another:
                            ...
                            elif abs(recording_event.start_date - prev_recording_event.start_date)< datetime.timedelta(seconds=60) :
                                try:
                                    logger.info('Combining: {0} {1} {2} {3}'.format(recording_event.lname, prev_recording_event.lname, recording_event.start_date, prev_recording_event.start_date))
                                except UnicodeEncodeError:
                                    pass
...
def generate_mediasite_schedule_class(cal_items1, msclass):
        ...
        while len(cal_items)>0:
            ...
            if overlap< datetime.timedelta(minutes=16) and last.mediasite_fldr == a.mediasite_fldr:
                ...
                if not a.combined_with_another and last.excluded ==a.excluded:
                    last.combine(a)
            elif not a.combined_with_another and overlap < datetime.timedelta(minutes=2) and last.mediasite_fldr != a.mediasite_fldr:
                last.cut_short=True
                new_sched.append(a)
            else:
                new_sched.append(a)
    ...
    new_sched2=s.query(ScheduledRecording).filter(ScheduledRecording.cyear == msclass.cyear).filter(ScheduledRecording.excluded == 0).filter(ScheduledRecording.combined_with_another == 0).filter(ScheduledRecording.start_date> datetime.datetime.now()).order_by(ScheduledRecording.start_date).all()
...
def construct_docs_message (messagelines, updateddocs, subscriber):
    messagelines.append("Navidile found these documents updated on Navigator.  Make sure you are logged in to Navigator <http://navigator.medschool.pitt.edu> to access them. \n")
    lastfolder=""
    for doc in updateddocs:
        if lastfolder != doc.folder_name:
            messagelines.append('\n')
            messagelines.append("==%s==" % ( removeNonAscii( doc.folder_name)))
            lastfolder = doc.folder_name 
        messagelines.append( "-{0} [{1}] <{2}> at {3}".format(removeNonAscii( doc.doc_name), removeNonAscii(doc.doc_ext), doc.full_url, doc.date_added))
    messagelines.append("\nTo unsubscribe to this alert, reply to this email with 'unsubscribe' in the message. Your last update was at {0}.".format(subscriber.last_update))
...
def construct_html_pagevids_all( msclass ):
    ...
    for course in s.query(Course).filter(Course.cyear==msclass.cyear).filter(Course.start_date< datetime.datetime.now()).order_by(desc(Course.start_date)).all():

        lines.append('<h3>%s<br>%s</h3>\n' % (course.name, get_info_line(course) ))

        lines.append('<table>\n')
        recs2 = s.query(Recording).filter(Recording.course_uid == course.unique_id).order_by(desc(Recording.rec_date)).all()
        for rec in recs2: 
            if not rec.navidile_url:
                lines.append('<tr><td>{3}</td><td>{2}</td><td><a href="{0}" rel="nofollow">{1}</a></td></tr>\n'.format( rec.mediasite_url, rec.name.replace(';', '<br>') , rec.rec_date.strftime("%Y-%m-%d"), rec.rec_date.strftime("%A")[0:3]))
            else:

                lines.append('<tr><td>{5}</td><td>{4}</td><td><a href="{0}" rel="nofollow">{1}</a> </td><td>[<a href = "{2}">mp3</a>]</td><td>[<a href = "{3}">navidile</a>]</td></tr>\n' .format ( rec.mediasite_url, rec.name.replace(';', '<br>+') , rec.podcast_url, rec.navidile_url, rec.rec_date.strftime("%Y-%m-%d"), rec.rec_date.strftime("%A")[0:3]))
...
class Course(Base):
    ...
    def __init__(self, name,  cyear,  course_id=None, navigator_url=None, mediasite_url=None, podcast_url=None, rec_exclude=None, auto_number=False, keep_updated=False):
        self.name = name
        self.cyear = cyear
...
class Document(Base):
    ...
    def __init__(self, folder, page, document, course ):
        ...
        self.doc_name = removeNonAscii(document['title'])
        self.full_url ="http://navigator.medschool.pitt.edu"+self.url
        if folder['displayName'] is None : 
		self.folder_name='NoName'
	else :
		self.folder_name=removeNonAscii(folder['displayName'])        
...
class NavidileWarning(Base):
    ...
    def __init__(self, subject, warningtxt, cyear, tonotify=True):
        self.subject = removeNonAscii(subject)
        self.warning = removeNonAscii(warningtxt)
        self.cyear = cyear
...
class Recording(Base):
    ...
    def __init__(self, idno,  name ="", mediasite_url="",  podcast_url="", navidile_url="", rec_date=None, course=None,   folder_id=None, pub_date =""):
        self.idno = idno
        self.name = name
        self.mediasite_url = mediasite_url
        self.podcast_url = podcast_url
...
class Exam(Base):
    ...
    exam_name = Column(String(225), primary_key=True)
    last_updated = Column(DateTime, nullable=True)
    subscriptions = Column(String(14), nullable=True)
    cyear = Column(Integer, nullable=True)

    
    def __init__(self, exam_name,  cyear):
...
class ScheduledRecording(Base):
    ...
    def __init__(self, calitem, ):
        ...
        self.lname= removeNonAscii(calitem.name).replace("Lecture: ", "").replace("Lecture ", "L").replace("  ", " ")
        self.l0name = "L%02d: %s" % (self.lec_id, calitem.name.replace("Lecture: ", "").replace("Lecture ", "").replace("  ", " "))
